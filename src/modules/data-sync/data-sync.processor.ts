...
export class DataSyncProcessor {
  ...
  async processSync(job: Job<SyncJobPayload>): Promise<void> {
    ...
    switch (job.data.type) {
      case SyncJobType.USER_PROFILE_UPDATE:
        await this.handleUserProfileUpdate(job.data);
        break;
      case SyncJobType.USER_SUMMARY_UPDATE:
        await this.handleUserSummaryUpdate(job.data);
        break;
      case SyncJobType.MATCH_FEEDBACK:
        await this.handleMatchFeedback(job.data);
        break;
      case SyncJobType.PERSONA_REFRESH:
        await this.handlePersonaRefresh(job.data);
        break;
      case SyncJobType.EMBEDDING_UPDATE:
        await this.handleEmbeddingUpdate(job.data);
        break;
      default:
  ...
  private async handleUserProfileUpdate(payload: UserProfileSyncPayload): Promise<void> {
    ...
    try {
      await axios.post(
        `${this.aiServiceUrl}/api/v1/user/sync-profile`,
        {
          user_id: userId,
          changes,
  ...
  private async handleUserSummaryUpdate(payload: UserSummarySyncPayload): Promise<void> {
    ...
    try {
      await axios.post(
        `${this.aiServiceUrl}/api/v1/user/sync-summary`,
        {
          user_id: userId,
  ...
  private async handleMatchFeedback(payload: MatchFeedbackSyncPayload): Promise<void> {
    ...
    try {
      await axios.post(
        `${this.aiServiceUrl}/api/v1/user/feedback-with-reasons`,
        {
          user_id: userId,
          match_id: matchId,
  ...
  private async handlePersonaRefresh(payload: PersonaRefreshPayload): Promise<void> {
    ...
    try {
      await axios.post(
        `${this.aiServiceUrl}/api/v1/user/refresh-persona`,
        {
          user_id: userId,
  ...
  private async handleEmbeddingUpdate(payload: EmbeddingUpdatePayload): Promise<void> {
    ...
    try {
      await axios.post(
        `${this.aiServiceUrl}/api/v1/user/update-embeddings`,
        {
          user_id: userId,
